...
const { commandData } = require("./slash/leave.js");
...
const init = async () => {
  ...
  for (const file of slashFiles) {
    ...
    client.commandArray.push(command.commandData);
    const { REST } = require("@discordjs/rest");
    const { Routes } = require("discord-api-types/v9");    
    const clientId = "1010486828261965864";
    const rest = new REST({ version: "9" }).setToken(process.env.DISCORD_TOKEN);
    try {
      logger.log("starting refreshing application commands");

      await rest.put(Routes.applicationCommands(clientId), {
        body: client.commandArray,
      });

      logger.log("Success reload application commands");
    } catch (error) {
      console.error(error);
    }
  }
